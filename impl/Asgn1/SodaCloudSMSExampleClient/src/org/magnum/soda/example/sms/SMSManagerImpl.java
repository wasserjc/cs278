		...
		ctx.registerReceiver(new BroadcastReceiver() {
			...
			public void onReceive(Context context, Intent intent) {
				...
				for (int i = 0; i < messages.length; i++) {
					...
					smsMessage.setContent(msg.getMessageBody());
 ...
 */
	...
	public void received(SMS sms){
		// create 'received' event to send to all listeners
		SMSEvent ev = new SMSEvent();
		ev.setEventType(EVENT_TYPE.RECEIVE);
		ev.setSms(sms);
		// iterate over all listeners and invoke smsEvent() method
		// to notify all listeners of the event
		for (int i = 0; i < listeners.size(); i++) {
			listeners.get(i).smsEvent(ev);
		}
	}
